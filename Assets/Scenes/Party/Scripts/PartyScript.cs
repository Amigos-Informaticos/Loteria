...
public class PartyScript : MonoBehaviour
{
	...
	public IEnumerator CheckIfWinner()
	{
		while (true)
		{
			...
			if (!_winner.Equals("NO WINNER") && !_winner.Equals("ERROR") &&
			    !_winner.Equals("ERROR. TIMEOUT") && !_winner.Equals("ROOM NOT FOUND"))
			{
				StopAllCoroutines();
				GoToPodium();
			}
		...
		while (true)
		{
			...
			while (counter < _room.Messages.Count)
			{
				this.taChat.GetComponent<TextMeshProUGUI>().text = this.taChat.text + "\n" +
					_room.Messages[counter].Key + ": " + _room.Messages[counter].Value;
				counter++;
			}
		}
	}
	...
	public void GenerateBoard()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				imgBoard[idBoardCard].GetComponent<Image>().sprite =
					this.CreateSpriteOfACard(_player.Board.Cards[i, j]);
				imgBoard[idBoardCard].GetComponent<Image>().name =
					_player.Board.Cards[i, j].ToString();
				idBoardCard++;
			}
		}
	}
